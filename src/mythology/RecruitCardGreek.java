...
public class RecruitCardGreek extends RecruitCard {
	...
	private RecruitCardGreek recruit;

	private JButton button;

	public RecruitCardGreek(ResourceDialog resource, PlayerBoard playerBoard,
		...
		label1 = new JLabel("Centaur");
		combo1 = new JComboBox<Integer>();
		cost1 = new JLabel("Wood=3 Favor=1");
		combo1.setSize(40, 10);

		label2 = new JLabel("Cyclops");
		combo2 = new JComboBox<Integer>();
		combo2.setSize(40, 10);
		cost2 = new JLabel("Food=3 Favor=3");

		label3 = new JLabel("Manticore");
		combo3 = new JComboBox<Integer>();
		cost3 = new JLabel("Food=2 Favor=2");

		label4 = new JLabel("Classical Greek Hero");
		combo4 = new JComboBox<Integer>();
		cost4 = new JLabel("Food=3 Gold=3");

		label5 = new JLabel("Toxotes");
		combo5 = new JComboBox<Integer>();
		cost5 = new JLabel("Food=1 Wood=1");

		label6 = new JLabel("Heroic Greek Hero");
		combo6 = new JComboBox<Integer>();
		cost6 = new JLabel("Food=3 Gold=4");

		label7 = new JLabel("Hydra");
		combo7 = new JComboBox<Integer>();
		cost7 = new JLabel("Gold=2 Favor=2");

		label8 = new JLabel("Minotaur");
		combo8 = new JComboBox<Integer>();
		cost8 = new JLabel("Food=2 Wood=2");

		label9 = new JLabel("Mythical Greek Hero");
		combo9 = new JComboBox<Integer>();
		cost9 = new JLabel("Gold=4 Favor=4");

		label10 = new JLabel("Hippokon");
		combo10 = new JComboBox<Integer>();
		cost10 = new JLabel("Food=1 Gold=1");

		label11 = new JLabel("Hoplite");
		combo11 = new JComboBox<Integer>();
		cost11 = new JLabel("Food=1 Wood=1");

		label12 = new JLabel("Medusa");
		combo12 = new JComboBox<Integer>();
		cost12 = new JLabel("Food=1 Favor=3");
		initializeUnits();
		...
		button.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (combo2.getSelectedIndex() != -1
					...
					favor = favor + ((Integer) combo2.getSelectedItem() * 3);
					unitsSelected = unitsSelected
							+ (Integer) combo2.getSelectedItem();
				}
				if (combo3.getSelectedIndex() != -1
						&& combo3.getSelectedIndex() != 0) {
					food = food + ((Integer) combo3.getSelectedItem() * 2);
					favor = favor + ((Integer) combo3.getSelectedItem() * 2);
					unitsSelected = unitsSelected
				...
				if (combo4.getSelectedIndex() != -1
					...
					if (player.getAge().equals("Classical")) {
						food = food + ((Integer) combo4.getSelectedItem() * 3);
						...
						JOptionPane
								.showMessageDialog(null,
										"You need to be in Classical age to recruit Classical Greek Hero");
						return;
					}
				}
				if (combo5.getSelectedIndex() != -1
						&& combo5.getSelectedIndex() != 0) {
					food = food + ((Integer) combo5.getSelectedItem() * 1);
					wood = wood + ((Integer) combo5.getSelectedItem() * 1);
					unitsSelected = unitsSelected
				...
				if (combo6.getSelectedIndex() != -1
					...
					if (player.getAge().equals("Heroic")) {
						food = food + ((Integer) combo6.getSelectedItem() * 3);
						...
						JOptionPane
								.showMessageDialog(null,
										"You need to be in Heroic age to recruit Heroic Greek Hero");
						return;
					}
				}
				if (combo7.getSelectedIndex() != -1
						&& combo7.getSelectedIndex() != 0) {
					gold = gold + ((Integer) combo7.getSelectedItem() * 2);
					favor = favor + ((Integer) combo7.getSelectedItem() * 2);
					unitsSelected = unitsSelected
							+ (Integer) combo7.getSelectedItem();
				}
				if (combo8.getSelectedIndex() != -1
						&& combo8.getSelectedIndex() != 0) {
					food = food + ((Integer) combo8.getSelectedItem() * 2);
					wood = wood + ((Integer) combo8.getSelectedItem() * 2);
					unitsSelected = unitsSelected
				...
				if (combo9.getSelectedIndex() != -1
					...
					if (player.getAge().equals("Mythic")) {
						gold = gold + ((Integer) combo9.getSelectedItem() * 4);
						favor = favor
								+ ((Integer) combo9.getSelectedItem() * 4);
						unitsSelected = unitsSelected
								+ (Integer) combo9.getSelectedItem();
					} else {
						JOptionPane
								.showMessageDialog(null,
										"You need to be in Mythic age to recruit Mythical Greek Hero");
						return;
					}
				}
				if (combo10.getSelectedIndex() != -1
						&& combo10.getSelectedIndex() != 0) {
					food = food + ((Integer) combo10.getSelectedItem() * 1);
					gold = gold + ((Integer) combo10.getSelectedItem() * 1);
					unitsSelected = unitsSelected
							+ (Integer) combo10.getSelectedItem();
				}
				if (combo11.getSelectedIndex() != -1
						&& combo11.getSelectedIndex() != 0) {
					food = food + ((Integer) combo11.getSelectedItem() * 1);
					wood = wood + ((Integer) combo11.getSelectedItem() * 1);
					unitsSelected = unitsSelected
							+ (Integer) combo11.getSelectedItem();
				}
				if (combo12.getSelectedIndex() != -1
						&& combo12.getSelectedIndex() != 0) {
					food = food + ((Integer) combo12.getSelectedItem() * 1);
					favor = favor + ((Integer) combo12.getSelectedItem() * 3);
					unitsSelected = unitsSelected
				...
				if (unitsSelected <= actionCard.getNumber()) {
					...
					JOptionPane.showMessageDialog(null,
							"Can't recruit more than " + actionCard.getNumber()
									+ " units!");
